		...
		public override void HandlePacket(BinaryReader reader, int whoAmI) {
			...
			switch (msgType) {
				...
				case PacketMessageType.UpdateRecordsFromServerToPlayer:
					...
					Main.LocalPlayer.GetModPlayer<RecordModPlayer>().RecordsForWorld?[recordIndex].NetReceiveRecords(reader);
					break;
				case PacketMessageType.RequestWorldRecords: // Multiplayer client --> Server
					...
					for (int i = 0; i < keyCount; i++) {
						...
						if (Systems.RecordSystem.WorldRecordsForWorld.FindIndex(x => x.BossKey == key) != -1)
							keysFound.Add(key);
					}
