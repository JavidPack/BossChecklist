...
namespace BossChecklist
{
	class BossUISystem : ModSystem
	{
			...
			if (playerChatIndex != -1) {
				layers.Insert(playerChatIndex, new LegacyGameInterfaceLayer("BossChecklist: Record Tracker Debugger",
					delegate {
						...
						if (NPCAssist.GetEntryInfo(BossChecklist.DebugConfig.ShowTimerOrCounter.Type) is not EntryInfo entry || entry.IsRecordIndexed(out int recordIndex) is false)
							return true;
		...
		private void DrawTooltipBackground(string text, Color textColor = default) {
			...
			int padd = 20;
			Vector2 stringVec = FontAssets.MouseText.Value.MeasureString(RemoveChatTags(text)); // remove chat tags for display text value
		...
		/// <summary>
		/// Iterates through a string value to detect and remove instances of chat tags.
		/// </summary>
